...
int main() {
    ...
    while(choice != 7) {
        ...
        else if(choice < 4) {           // Otherwise, choice was a game of sort.
...
// It will create a new player accound and append it to the file.
void register_new_player() {
    int fd;
...
int find_the_ace() {
    ...
    while(invalid_choice) {         // Loop until valid choice is made. 
        ...
        if(choice_two == 'c') {     // Change pick.
            ...
            printf("Your card pick has been chnaged to card %d\n", pick+1);
        }
    }
